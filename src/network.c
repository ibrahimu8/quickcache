...
int network_put(const hash_t key, const char *file_path) {
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, cfg->timeout_seconds * 2); // Longer timeout for uploads
    if (headers) {
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
    }
...
void network_put_async(const hash_t key, const char *file_path) {
    ...
    strncpy(upload_queue[upload_queue_size].file_path, file_path, sizeof(upload_queue[upload_queue_size].file_path) - 1);
    upload_queue_size++;
